...
\begin{equation}
  m=\binom n2 .
\end{equation}
For $u\in V$, write $\indeg(u)=|\{v:(v,u)\in A\}|$ and $\outdeg(u)=|\{v:(u,v)\in A\}|$ for the in-degree and out-degree of $u$ in $T$, and set
\begin{equation}
...
The pairwise posterior probabilities of the designated events encode the tournament exactly.
...
\begin{proof}
Let $B_u$ be the block containing $X_u$.  By Lemma~\ref{lem:independence}, the random variables $B_u$ are mutually independent and uniform on $\{1,\ldots,R\}$.
...
\end{align}
Substituting $c_{uv}=R/2+1$ when $(u,v)\in A$ and $c_{uv}=R/2-1$ when $(v,u)\in A$ (Lemma~\ref{lem:profile}(i)), and applying Lemma~\ref{alem:normalized} with $\varepsilon=+1$ and $\varepsilon=-1$ respectively, yields the two cases of the statement.  Since $R\ge 2$, the probability exceeds $1/2$ exactly when $(u,v)\in A$, so the posterior majority direction is the arc direction.  Finally, writing $\Prb(X_u<X_v\mid\hat t)=\tfrac12+s_{uv}$ with $s_{uv}=\pm 1/R^2$, Lemma~\ref{alem:centered} gives the decision weight $2|s_{uv}|=2/R^2$ and the constant term $\tfrac12-|s_{uv}|=\tfrac12-\tfrac1{R^2}$.
\end{proof}

\subsection{Filler pairs are exactly neutral}

The remaining task is to show that every pair involving a filler has posterior probability $1/2$ in each direction.  This holds for two genuinely different reasons: filler-filler pairs are neutral by a symmetry argument, while filler-designated pairs are neutral by an exact cancellation.  We record the two mechanisms as separate lemmas.
...
\begin{proof}
Fillers have identical noise distributions and the posterior is invariant under relabeling them, so
\begin{equation}
  \Prb(X_f<X_{f'}\mid \hat t)=\Prb(X_{f'}<X_f\mid \hat t).
\end{equation}
Because $\sigma$ is a permutation there are no ties, so these two probabilities sum to one; being equal and summing to one, each equals $1/2$ by Lemma~\ref{alem:common}(iii).  Note that this argument uses nothing about the tournament: it holds for any construction in which the fillers are exchangeable.
\end{proof}
...
\begin{proof}
Unlike the filler-filler case, this is not a symmetry: a designated event and a filler are not exchangeable, since $d_u$ carries the graph signal $\delta_u$.  The probability must instead be computed, and neutrality emerges from a cancellation.

Fix a designated event $d_u$ and a filler $f$.  All fillers have the same noise distribution, so relabeling them leaves the posterior unchanged; hence
\begin{equation}
...
Next compute the designated-designated terms.  By Lemma~\ref{lem:designated} applied to the ordered pair $(v,u)$: if $(v,u)\in A$, then $v$ is an in-neighbor of $u$ and
\begin{equation}
  \Prb(X_v<X_u\mid \hat t)=\frac12+\frac1{R^2},
\end{equation}
while if $(u,v)\in A$, then
\begin{equation}
  \Prb(X_v<X_u\mid \hat t)=\frac12-\frac1{R^2}.
\end{equation}
...
\end{equation}
By Lemma~\ref{alem:common}(i) with $z=\delta_u/R^2$, the right-hand side equals $(2nR-n)/2=q/2$, using $q=2nR-n$.  Since $q=n(2R-1)>0$ (as $n\ge 4$ and $R\ge 1$), Lemma~\ref{alem:common}(ii) gives $\theta_u=1/2$.  Finally, since there are no ties, $\Prb(X_u<X_f\mid \hat t)=1-\theta_u=1/2$ as well.  The neutrality is an arithmetic consequence of the calibration of $\alpha_u$ against the profile rank sums: the per-vertex signal $\delta_u$ appears in both $\E[X_u\mid\hat t]$ and the designated-designated sum, and cancels.
\end{proof}

Combining Lemmas~\ref{lem:fillerfiller} and~\ref{lem:fillerdesignated}, every pair involving a filler has posterior probability exactly $1/2$ in each direction, so by the final clause of Lemma~\ref{lem:majority} it contributes exactly $1/2$ to the posterior expected Kendall-tau distance, independent of the candidate order $\hat\pi$.
...
\begin{itemize}
\item Each filler-involved pair has posterior probability exactly $1/2$ in each direction (Lemmas~\ref{lem:fillerfiller} and~\ref{lem:fillerdesignated}), so by the final clause of Lemma~\ref{lem:majority} it contributes exactly $\tfrac12$, independent of $\hat\pi$.  The total over these pairs is $\left(\binom L2-m\right)\tfrac12$.
...
\end{equation}
by Lemma~\ref{lem:designated}.  The posterior majority direction for $\{d_u,d_v\}$ is $d_u$ before $d_v$ exactly when $(u,v)\in A$ (Lemma~\ref{lem:designated}), so $z_{uv}=1$ exactly when the corresponding tournament arc is backward under $\pi_{\hat\pi}$; hence
\begin{equation}
