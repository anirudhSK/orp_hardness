...
% development is sorry-free modulo the three cited/elementary axioms, which are
...
``ORP is NP-hard'' is assembled from the proven core together with three
explicitly stated axioms: that FAST is NP-complete (textbook, cited), that a
correct polynomial-time many-one reduction from an NP-complete source yields
NP-hardness of the target (a meta-fact), and one generic, problem-independent
permutation fiber-count. \texttt{\#print axioms orp\_np\_hard} surfaces exactly
these.
...
\leanok
A \emph{tournament} on $V = \{1,\dots,n\}$ ($= \mathrm{Fin}\,n$) is a relation
...
\begin{theorem}[Permutation fiber-count \emph{(axiom: generic, problem-independent)}]
\label{ax:fiber_count}
\lean{LeaFrontier.OrderingRecovery.sum_perm_comp_embedding}
\leanok
For any embedding $e : \mathrm{Fin}\,n \hookrightarrow \mathrm{Fin}\,L$ and any
$F$,
\[
  \sum_{\sigma : \mathrm{Perm}(\mathrm{Fin}\,L)} F(\lambda u.\,\sigma(e\,u))
  = (L-n)!\, \cdot
    \sum_{\varphi : \mathrm{Fin}\,n \hookrightarrow \mathrm{Fin}\,L} F(\lambda u.\,\varphi\,u):
\]
each injective placement of $n$ marked indices extends to a full permutation in
exactly $(L-n)!$ ways. \emph{Axiomatized} (\texttt{PermFiber.lean}): true,
elementary, and entirely independent of the ORP construction; isolated so the
genuinely novel content is proved on top of it. (TODO: discharge / upstream to
Mathlib.)
\end{theorem}
...
Lemma~\ref{lem:reduction_correct}. Hence ORP is NP-hard. \texttt{\#print axioms
orp\_np\_hard} is exactly \texttt{[propext, Classical.choice, Quot.sound,
fast\_np\_complete, reduction\_yields\_np\_hard, sum\_perm\_comp\_embedding]}.
\end{proof}
